...
async def sing_up(message):
    ...
    await RegistrationUserStates.username.set()


@dp.message_handler(state=[RegistrationUserStates.username])
async def set_username(message, state):
    while True:
        if is_include(username=message.text):
            print(message.text, type(message.text))
            await state.update_data(username=message.text)
            break
        await RegistrationUserStates.username.set()
    await message.answer('Введите свой email:')
    await RegistrationUserStates.email.set()


@dp.message_handler(state=[RegistrationUserStates.email])
async def set_email(message, state):
    print(message.text, type(message.text))
    await state.update_data(email=message.text)
    await message.answer('Введите свой возраст:')
    await RegistrationUserStates.age.set()


@dp.message_handler(state=[RegistrationUserStates.age])
async def set_age(message, state):
    await state.update_data(age=int(message.text))
    data2 = await state.get_data()
    add_users(data2['username'], data2['email'], data2['age'])
